...
def make_onehot_label(label, num_classes):
    ...
    for c in range(num_classes):#0,1,2,3
        onehot_encoding.append(label == c)#应该是还要确认，h,w,c,类标
...
def image_norm_01(img):
    img-=np.min(img)
    img/=np.max(img) #0.95分位值
    return ((img - mean) / std) * mask#mask取值0~1
    # return (img - mean) / std

def image_norm_dis(img):
    mask = (img != 0)
    mean = np.mean(img[mask])
    std = np.std(img[mask])
    return ((img - mean) / std)* mask #mask取值0~1
...
def vote(segmentation_sets):
    #这个有影响吗
    onehot_segmentation_sets = [make_onehot_label(seg, 4) for seg in segmentation_sets]#0512
    #nehot_segmentation_sets = [make_onehot_label(seg, 4) for seg in segmentation_sets]
    vote_seg = np.argmax(sum(onehot_segmentation_sets), axis=-1)
    vote_seg = np.array(np.expand_dims(vote_seg, axis=-1), dtype=np.uint8)
    return vote_seg

def vote5(segmentation_sets):
    #这个有影响吗
    onehot_segmentation_sets = [make_onehot_label(seg, 5) for seg in segmentation_sets]#0816
    #nehot_segmentation_sets = [make_onehot_label(seg, 4) for seg in segmentation_sets]
    vote_seg = np.argmax(sum(onehot_segmentation_sets), axis=-1)
    vote_seg = np.array(np.expand_dims(vote_seg, axis=-1), dtype=np.uint8)
    return vote_seg
